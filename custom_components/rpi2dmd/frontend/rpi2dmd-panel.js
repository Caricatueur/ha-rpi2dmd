...
class Rpi2dmdPanel extends HTMLElement {
  ...
  _quickControls() { const brightness=this._brightness(); return `<div class="controls"><label class="brightness-control"><span class="brightness-line"><span>Luminosité</span><output class="brightness-value" id="brightness-value" for="brightness">${brightness} %</output></span><input type="range" min="0" max="100" step="5" id="brightness" value="${brightness}"></label>${["clock","date","weather","gif","mqtt"].map(f=>`<label class="toggle"><input type="checkbox" data-flag="${f}" ${this._flag(f)?"checked":""}> ${this._labelFlag(f)}</label>`).join("")}</div>`; }
  ...
  _bind() {
    ...
    const bright=this.shadowRoot.querySelector("#brightness"); const brightValue=this.shadowRoot.querySelector("#brightness-value"); if(bright){const updateLabel=()=>{if(brightValue) brightValue.textContent=`${bright.value} %`;}; bright.addEventListener("input",updateLabel); bright.onchange=()=>this._updateDisplay({brightness:{schedule:[{hour:new Date().getHours(),value:Number(bright.value)}]}});}
